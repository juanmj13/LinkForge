...
// --- render ---
function totalPages() {
  return Math.max(1, Math.ceil(eventsData.length / PAGE_SIZE));
}

function renderTable() {
  const tbody = document.querySelector<HTMLTableSectionElement>('#tbl-events tbody');
  if (!tbody) return;
  ...
  if (slice.length === 0) {
    ...
    const info = document.getElementById('evt-pager-info');
  ...
  tbody.querySelectorAll<HTMLButtonElement>('.btn-view').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // console.log('Ver id:', btn.dataset.id);
    });
  });
...
function renderPager() {
  const pagesWrap = document.getElementById('evt-pager-pages');
  if (!pagesWrap) return;
